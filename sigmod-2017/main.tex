...
We are currently implementing large scale use cases that require a distributed solution.
The distributed version is based on the architectural design principles described in the paper.
It includes a distributed Stochastic Gradient Descent, a data manager that works with distributed file systems and distributed in-memory data structures, and a model manager that partitions the machine learning model (if its size exceeds master node's memory limit) across several nodes and can update it based on new training data.
Finally, we would like to explore other optimization strategies such as batching of prediction queries, caching of query results and more advanced methods for sampling of historical data in order to investigate their effect on the performance of the system.
