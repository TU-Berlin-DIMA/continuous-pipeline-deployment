...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{../images/use-case.png}
...
%Given the lack of systems to support model deployment and maintenance, we have set out to design a system that not only supports deployment and maintenance of machine learning models but also provides %constant incremental and batch updates while the system is running. 
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{../images/continuous.png}
...
the \textit{error\_rate} method returns the mean squared error rate of the model using the provided test dataset.
...
The model manager creates snapshots of the model in two different scenarios; after a series incremental updates are made and after each training iteration.
...
We set the number of hidden layers to 50 and use a softmax \cite{bishop2006pattern} output function.
...
Figure \ref{fig:movie-lens-100k-buffer-size-mse} shows the mean squared error for different buffer sizes for 100k MovieLens dataset. 
Smaller buffer sizes yield lower error rates, but the figure shows that the difference is small.
Especially, when considering the affect the buffer size has on the running time.
...
The gap in error rate between offline+online and offline-only is much smaller than that of movie lens datasets.
Unlike movie lens datasets, \textit{MNIST} does not contain a shift in distribution and that is why both offline+online and offline-only performance is rather similar.
Offline+online performs slightly better because of the extra training items that the underlying neural network is exposed to.
Full-retraining and continuous methods behave similarly as well and unlike Movie Lens datasets, full-retraining has a more or less consistent error rate, that is the error rate does not increase after a retraining of the model.
Similar to offline+online's case, since there is no change in distribution of the data, the error rate does not degrade after a retraining.
Continuous method manages to decrease the error rate as more training iterations are performed.
As more training iterations are executed, the underlying neural network's parameter converges and as a result the error rate on the test set decreases.
Both continuous and full-retraining method start to perform better than the baseline after a training iterations (retraining in full-retraining case).
...
%new paragraph
To demonstrate our system's ability to incorporate different machine learning models and workloads, we implemented two use cases: recommender system and Image classifier.
The recommender system was tested on different workloads and showed not only it scales well but also it also adopts to changes in data distribution by continuously updating the model.
Image classifier use case as well showed how we can implement and use neural networks within our system to create a model that not only can answer prediction requests quickly, but is also able to make incremental and batch updates to further increase the model quality without creating much overhead.
% end of new paraphraph
Our experiments also showed the continuous training of the machine learning models is much faster than full retraining of them, which is the most common approach in deployment and maintenance of machine learning models. We showed that not only continuous training requires less resources but it also produces models with lower error rates that adopt faster to changes in the dataset.
