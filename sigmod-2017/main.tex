...
\begin{abstract}
Machine learning model deployment is the last step in any machine learning pipeline.
...
Our method takes advantage of the stochastic and iterative nature of SGD based machine learning models, and not only applies incremental learning to constantly update the machine learning model but also eliminate the need for offline retraining without affecting the performance.
We have observed that the process of SGD optimization can be integrated into the lifeline of the model.
Each iteration of SGD is lightweight and can be executed while the system is running without interrupting the normal flow.
