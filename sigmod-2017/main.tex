...
\title{Continuous Training of SGD based Machine Learning Models after Deployment}
...
\begin{abstract}
Machine learning model deployment is the final stage of machine learning pipelines.
Typically, a machine learning model is first trained using historical data residing on disk. 
Once the training is done, the model is deployed into a system where it can answer prediction queries in a realtime and reliable fashion.
If new training data becomes available while the system is running, these models are updated.
...
In this paper, we propose a novel deployment method for Stochastic Gradient Descent based machine learning models.
Our method takes advantage of the stochastic and iterative nature of SGD based machine learning models, and not only applies incremental learning to constantly update the machine learning model but also eliminate the need for offline retraining by replacing it with mini-batch updates without affecting the performance.
We have observed that the process of SGD optimization can be integrated into the lifeline of the model.
One iteration of SGD is lightweight and can be executed while the system is running without interrupting the normal flow.
Therefore, instead of retraining we are constantly applying mini batch updates to the model.
Our experiments have shown that our deployment method can achieve up to an order of magnitude faster model update without degrading the quality of the model and in some case even increasing the quality due to faster adoption to changes in data distribution. 
...
% when to schedule a new iteration
...
% adjusted based on the feedback from quality control ? 
...
% every new iteration scheduled will be followed by a quality control
...
% it is not based on any computation model
...
% underlying engine can be anything as long as iterative optimization methods are supported
...
% joining data from disk with the the stream inside the buffer is not trivial
