...
However, it only works with batch data and does not provide support for realtime queries. 
As a result it does not support incremental learning.
However, our system is designed to work in a dynamic environment where it answers prediction queries in realtime and incrementally updates the model when required.
...
Furthermore, our system can work with any machine learning model that uses stochastic gradient descent as it's optimization algorithm.
...
Although SGD is typically used to train models on static datasets, our proposed system uses it to perform both incremental and batch updates to the deployed model.
Every successive iteration of SGD is scheduled to run while the machine learning model is also answering prediction queries.
...
To demonstrate our system's ability to incorporate different machine learning models and workloads, we implement two use cases: recommender system and logistic regression classifier.
...
\todo[inline]{explain the result of the other experiments}
