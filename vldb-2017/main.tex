...
%Volker
Once a machine learning model is trained on an initial dataset it is deployed into a system where it can answer prediction queries in a real-time and reliable fashion.
...
Furthermore, to ensure higher quality of prediction and better adaptation to changes in data distribution, models are retrained periodically.
...
Figure \ref{fig:velox-work-flow} demonstrates this deployment approach.
...
\todo[inline]{How is the info in this subsection ever used again?}
...
While logistic regression models aim to find parameters of a function that accurately fit the data points to the labels, SVM tries to find a hyperplane that separates the data points belonging to different classes. 
In many cases, both type of models work equally well, but depending the data and its complexity one method may be preferred to the other. \todo[inline]{Too generic, can we be more specific?}
...
The success of the Google Brain team was also instrumental to popularizing neural networks in the machine learning community \cite{sutskever2014sequence, mikolov2013efficient}.
\todo[inline]{Can we add info on their success?}
...
This explains why the Baseline method performs poorly.
...
The training iterations are executed, the faster the underlying neural network's parameters converge and as a result the error rate on the test set decreases.
...
Model deployment and serving is an important part of the data analytics pipeline.
In order for the deployed models to adapt to changes in the data distribution, they are incrementally updated.
However, incremental updates alone is not always sufficient and to maintain model quality, periodic retraining is required. 
...
Some systems \cite{crankshaw2014missing} perform incremental update and periodic retraining to ensure the quality of the model does not decrease.
Retraining is a resource intensive process which completely neglects any updates made to the model so far.

We propose a system for deploying and maintaining machine learning models that are trained using stochastic gradient descent optimization (SGD).
SGD is an iterative optimization process where in each iteration a sample of the data is used to update the machine learning model.
...
SGD is one of the most common optimization techniques for large datasets and has been successfully used in different domains such as neural networks, recommender systems, regression, and clustering models. 
This makes our system applicable to a wide range of machine learning models beyond the ones demonstrated in our evaluation section.
...
We show that not only continuous training requires less resources but it also produces models with lower error rates that adopt faster to changes in the data.
