...
Baseline method's error rate is increasing with a steady rate as it cannot adapt to the changes in the data.
...
Velox, Baseline+ and Continuous maintain the same error rate until the first retraining of Velox, after which due to possible overfitting, Velox error rate starts to increase.
...
Figure \ref{fig:cluster-training-time} shows the total training time for URL Reputation and Higgs.
...
For all the evaluated datasets, the Continuous deployment method achieves the lowest average error rate (for classification datasets) and average mean squared error (for Recommender system datasets).
...
Since both Velox and Continuous methods perform incremental training of the model after it is deployed, they both manage to handle the concept drift in the data.
...
In Velox deployment method, the underlying model cannot adapt to the changes in distribution as quickly, since the retraining procedure cannot be executed frequently as it requires a lot of resources.
Baseline deployment method's performance is very poor in datasets  with concept drift.
This is expected, as the underlying model in Baseline approach is only trained on an initial dataset and is not able to adapt to the changes in the data.
...
In our current prototype we do not tackle the problem of the trade off between prediction latency and prediction accuracy.
