\documentclass[sigconf,edbt]{acmart-edbt2019}



% Copyright
\setcopyright{rightsretained}
%\settopmatter{authorsperrow=1}
% DOI
\acmDOI{}

% ISBN
\acmISBN{978-3-89318-081-3}
%Conference
\acmConference[EDBT 2019]{22nd International Conference on Extending Database Technology (EDBT)}{March 26-29, 2019}{Lisbon, Portugal} 
\acmYear{2019}

\settopmatter{printacmref=false, printccs=false, printfolios=false}

\pagestyle{empty} % removes running headers
\let\ACMcomment\comment
\let\comment\relax
\usepackage[authormarkup=superscript, commentmarkup=margin, final]{changes}
\usepackage[disable]{todonotes}
\definechangesauthor[name ={Reviewer 1}, color = blue]{r1}
\definechangesauthor[name ={Reviewer 2}, color = blue]{r2}
\definechangesauthor[name ={Reviewer 3}, color = blue]{r3}
\usepackage[moderate]{savetrees}
\usepackage{multirow}
\usepackage{diagbox}
\usepackage{algorithm}% http://ctan.org/pkg/algorithms
\usepackage{algpseudocode}% http://ctan.org/pkg/algorithmicx
\usepackage{url}
\usepackage{lipsum}% http://ctan.org/pkg/lipsum
\usepackage{graphicx}% http://ctan.org/pkg/graphicx
\usepackage{adjustbox}
\usepackage{color,soul}
\usepackage{float}
\usepackage{amsmath}
\usepackage[outdir=./figures/]{epstopdf}
\usepackage{listings}
\PassOptionsToPackage{table}{colorx}
\usepackage{booktabs}
\usepackage{subcaption}
\usepackage{upquote}
\usepackage{tikz}
\DeclareMathOperator*{\argmin}{arg\,min}
\DeclareMathOperator*{\argmax}{arg\,max}
\newcommand*\circled[1]{\tikz[baseline=(char.base)]{\node[shape=circle,draw,inner sep=2pt] (char) {#1};}}            
\algrenewcommand\algorithmicrequire{\textbf{Input:}}
\algrenewcommand\algorithmicensure{\textbf{Output:}}
 
\soulregister\textit7
\soulregister\cite7
\soulregister\ref7
\soulregister\pageref7
\begin{document}

%
%\lstset{numbers=left, numberstyle=\small, numbersep=8pt, frame = single, language=Pascal, framexleftmargin=15pt}

\title{Continuous Deployment of Machine Learning Pipelines}

\author{Behrouz Derakhshan, Alireza Rezaei Mahdiraji, Tilmann Rabl, and Volker Markl\\ }
\affiliation{ $  $\\ {\Large DFKI GmbH       \hspace{2.2cm}     Technische Universität Berlin}}
\affiliation{$  $\\{\large \{behrouz.derakhshan, alireza.rm, tilmann.rabl, volker.markl\}@dfki.de}} 
%\author{Behrouz Derakhshan}
%\affiliation{%
%  \institution{DFKI, Germany}
%}
%\email{behrouz.derakhshan@dfki.de}
%
%\author{Alireza Rezaei Mahdiraji}
%\affiliation{%
%  \institution{DFKI, Germany}
%}
%\email{alireza.rezaei_mahdiraji@dfki.de}
%
%\author{Tilmann Rabl}
%\affiliation{%
%  \institution{DFKI, Germany}
%}
%\additionalaffiliation{
%\institution{TU Berlin, Germany, rabl@tu-berlin.de}
%}
%\email{tilmann.rabl@dfki.de}
%
%\author{Volker Markl}
%\affiliation{%
%  \institution{DFKI, Germany}
%}
%\additionalaffiliation{%
%  \institution{TU Berlin, Germany, volker.markl@tu-berlin.de}
%}
%\email{volker.markl@dfki.de}


\input{sections/abstract}
\maketitle

%\keywords{Machine Learning Model Management; Stochastic Gradient Descent; Machine Learning Systems}

\input{sections/introduction}
\input{sections/background}
%\input{sections/ml-deployment-approaches}
\input{sections/continuous-training-and-serving}
\input{sections/architecture}
\input{sections/evaluation}
\input{sections/related-work}
\input{sections/conclusion}

\bibliographystyle{ACM-Reference-Format}
{\fontsize{8}{4}\selectfont \bibliography{references}}
\end{document}


