...
\todo[inline]{Find a reference for incremental training}
\hl{Proactive training is a form of incremental training [REF] which is limited to SGD-based models.
In our deployment platform, one can replace the proactive training with other forms of incremental training.
However, since the combination of the data sampling and the proactive training is similar to the mini-batch SGD procedure, we can guarantee the same regret bound as online stochastic gradient descent} \cite{zhang2004solving}.
\todo[inline]{I need to make sure the last sentence has a proper structure (guarantee a regret bound or provide a regret bound or guarantee regret bound convergence ... ?}
