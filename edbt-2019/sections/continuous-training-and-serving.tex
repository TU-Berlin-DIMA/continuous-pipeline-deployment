...
\end{aligned}
\end{equation}
\todo[inline]{Fixed the equation 4 and 5: changed $H_{m+1}$ to $H_{m}$ R2:11}
...
Then, in every iteration, we randomly sample points from the dataset (Line 4), compute the gradient of the loss function $J$ (Line 5), and finally update the model based on the value of the gradient and the learning rate (Line 6).}
