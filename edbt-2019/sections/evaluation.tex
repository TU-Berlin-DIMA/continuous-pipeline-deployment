...
\todo[inline]{This experiment is a superset Experiment 3. We should replace it and combine the analysis from the experiment 'Optimization effect' and the 'Dynamic Optimization'}
...
To demonstrate the effect of the dynamic optimization empirically, we execute the deployment scenario for both the URL and the Taxi dataset with different sampling strategies (i.e., time-based, uniform, and window-based) with different materialization rate.
We define the materialization rate as the ratio of the number of materialized chunks over the total number of chunks (the total number of chunks for the URL and Taxi datasets are 12,000 and xxx respectively).
...
\todo[inline]{note: this experiment is executed on only 20 days of the url data (out of 120) and the numbers are not final. 
Also, I think the increase from 0.6 to 1.0 materialization rate is because I'm running the experiment on my laptop and the Spark's master memory becomes full which slows down Spark.}
\begin{figure}[!h]
\centering
...
At materialization rate of 0.0, the platform has to rematerialize every chunk, regardless of the sampling strategy.
At materialization rate of 1.0, the platform utilizes the chunks without any rematerialization.
Therefore, the total deployment cost for the different sampling strategies is equal when materialization rate is 0.0 and 1.0.
As expected, when we increase the materialization rate (more chunks are materialized), the total deployment time decreases.
However, the decrease in the time-based and window-based sampling is larger.
Due to the nature of the sampling strategy, time-based and window-based sampling have higher probabilities of selecting chunks from the materialized chunks.

This experiment shows dynamic materialization can still decrease the total execution time, especially for the time-based and window-based sampling, in presence of limited storage resources.
