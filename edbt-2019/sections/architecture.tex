...
T' = S * T * pr * pl
\end{equation}
where $T'$ is the time in seconds when the next proactive training is scheduled to execute, $T$ is the length of the execution time (in seconds) of the previous proactive training, $pl$ is the average prediction latency (second per item), and $pr$ is the average number of prediction queries per second (items per second).
