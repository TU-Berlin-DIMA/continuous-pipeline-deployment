...
\subsection{Tuning Periodical Deployment}
\hl{Two sets of hyperparameters control the performance of the periodical deployment approach.
The first set, we call the deployment hyperparameters, consists of 2 hyperparameters, which defines how much of the historical data should be used in each retraining and how often each retraining should be performed.
The second set of hyperparameter, called the training hyperparameters, is to tune the retraining of the model.
...
There are several existing approaches for selecting the training hyperparameters, such as grid search, random search, and sequential model based search [REF].
The deployment hyperparameters are typically selected to fit the specific use case.
For example, in many of the real-world use cases, a model is retrained using the entire existing data on a daily basis.
