...
\todo[inline]{R2: The first contribution item says the proposed method works for any SGD based models. However, the API in Listing 1 does not consider unsupervised models which can also be trained using SGD, like RBM (restricted boltzmann machine). BD: I think the contribution is still valid, we just have to clarify that the prototype only supports supervised learning for now.}
\begin{itemize}
...
\todo[inline]{R1: In Section 3.1, the last sentence is a very strong claim (that Async SGD works as well as synchronous SGD). We know this to be true only in limited cases, and these techniques often require careful tuning of hyper-parameters to work well.}
