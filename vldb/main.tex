...
\maketitle
\begin{abstract}
A data analytics process is a pipeline comprised of several steps, from source selection, data preparation, feature engineering, to model training.
...
\todo[inline]{TR: This is not clear. This sounds like all systems do online training and periodic batch training.
BD: That what I mean to say here. I have added the word 'current' to make it more clear.}
While the system is running, new training data, both in the form of stream sources and batch datasets, may become available.
To maintain a high quality of prediction, the current deployment systems perform online training as well as periodical batch retraining.
...
Our method utilizes the iterative nature of SGD and replaces the retraining process with a series of consecutive SGD iterations.
...
\todo[inline]{Should I use these key words??}
...
\end{figure}
\todo[inline]{TR: How does the online learning influence the model? 
BD: Online learning tries to maintain the quality as best as it can, but a retraining still have be to executed to make sure the error does not degrade }
...
\todo[inline]{TR: Maybe we should talk about the process here.
BD: The deployment process? I'm not really sure how to summarize in 1 2 sentences }
...
\todo[inline]{TR: Maybe we should include this in the use case (the way the Zalando people explained it)
BD: It was included in the use case in previous versions, but I moved it here to indicate this is a secondary objective of the paper}
