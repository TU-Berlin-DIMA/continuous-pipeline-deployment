...
\todo[inline]{Which one is better in the first sentence: training and managing .... deploying and maintaining OR training and managing ... deployment and maintenance}
...
\todo[inline]{BD: Is this sentence clear? What I mean to say is that since the items are arriving one at a time, both sampling and scheduling rate can be set to exact values. In Spark, because of micro-batching, the scheduling and sampling are approximates rather than exact values}
...
In our local experiments, the entire resources of the computing machine were being consumed to perform the training.
...
\todo[inline]{Is the first paragraph necessary? If I remove it, should extend the length of the conclusion?}
...
\todo[inline]{Can I directly use the term "SGD" or is it better to redefine the abbreviation in the conclusion section}
We propose a system for deploying and maintaining machine learning models that are trained using stochastic gradient descent (SGD) optimization.
