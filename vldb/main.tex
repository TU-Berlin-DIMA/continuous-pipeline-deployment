...
\todo[inline]{TR: Seems like you need to clarify what is current, what is your solution, what is the problem. If a reader is left wondering, we have a problem.
BR: please see the highlighted section .. }
\hl{However, incremental updates alone are not enough to maintain the quality of the CTR prediction model.
...
\todo[inline]{TR: IF you do not have an experiment for this claim, then you should make it secondary or remove it. Which sentence do you mean?
BD: So I removed the original paragraph about the new batch datasets from the use case description and added the highlighted paragraph instead. }
\hl{Furthermore, while the deployment system is serving the model, new batch training datasets may become available.
...
\todo[inline]{TR: I would choose the first and repeat models. BTW: how do current systems manage models?

BD: By manage, I meant the model management during training (i.e. MLBase) which automatically trains multiple models for the same task and chooses the best one. I have removed "manage", since it seems a bit confusing to have it here.}
\hl{Traditional machine learning systems focus solely on training models and leave the task of deploying and maintaining these models to the users. }
...
\todo[inline]{TR: Question is, do we need this here, do you already have this earlier? If you have it earlier, no need to repeat. Same for the one below.
BD: Please see the next todo on top of Section 4}
...
\todo[inline]{TR: What I think he means is that you should make clear that you can use a broad variety of algorithms although you restrict yourself to SGD. This is along the lines of what I commented earlier. Important point is to either say that this is not a limitation or what the limitation is.
BD: I tried to justify the importance and popularity of SGD in Section \ref{sgd} (specially in \ref{subsec:ml-models-sgd}) so that by this point in the paper, it would be clear to the readers that by focusing on SGD we are not imposing limitations on the system. Instead, we are improving a method (SGD) that is very common in Machine Learning. In previous versions there was a sentence about SGD being very common, but I removed it because it would repeat the same point from previous chapter.}
...
\todo[inline]{Are the figure captions OK?}
...
\todo[inline]{TR: What you mean to say is not in the sentence in my mind. :)Maybe just add this explanation ?
BD: I guess I was overexplaining things.. I wrote the highlighted sentence instead. }
...
\hl{In our evaluations, we use Version1, to investigate the effect of scheduling and sampling rate on the running time and quality of the models.}
...
\todo[inline]{TR: I don't fully understand this.
BD: Is this more clear? }
\hl{For these two datasets, we use an evolving test set to evaluate the performance of the deployment methods.
We start with an empty test set.
Upon receiving new test items, we add them to the current test set and calculate the error (we call this a test increment).}
...
\todo[inline]{TR: Is this shown in the figure? 
BD: Maybe the highlighted section better described what I am trying to explain
Also, where do you analyze 5b?
BD: I moved the explanation of Figure 5b a few paragraphs forwards where I am explaining the effect of sampling rate.
}
...
Increasing the buffer size from 500 to 5000 decreases the running time by a factor of 5 \hl{while the MSE is only decreased slightly.}
...
\todo[inline]{TR: Over which baseline?}
\hl{Increasing the sampling rate from 0.1 to 1.0, increases the running time by a factor of 5.}
\todo[inline]{TR: How much is this dataset dependent? I cannot see much in the two figures, since everything is interacting too much

BD: In the next few paragraphs (starting from  "Tuning parameters based on error rate") I explain that this is a model dependent. I use Figure 7 to explain that in certain models (NNs) these parameters seem to have no effect on the quality of the model}
...
\todo[inline]{TR: Is this not the main part of the work in a long running setup?

BD: Since I'm focusing on eliminating the offline retraining of Velox, I excluded the time spent for online learning. I don't think including the online learning time here, makes much sense. That would be part of the delay that I have mentioned as future work. Moreover, both Velox and Continuous perform online learning anyway, so adding the incremental training time will just increase the total time by a constant factor in both deployment methods.}
...
\todo[inline]{I'm running a new set of experiments for URL Reputations. The result will looks the same, but I have increased the streaming time to match Higgs.}
...
BD: It should be doable. I ll try to find out how (probably need to use some internal spark reporting APIs)}
