...
\todo[inline]{Goal}
A deployment platform must be robust and accommodate many different types of machine learning models and pipelines.
...
Online learning adapt the model to the new training data and provide an up-to-date model.
However, purely online deployment approach introduces instability, degrades the quality over time, and is not always effective.
...
Despite this drawback, in some applications, retraining the model is still critical, as even a small increase in the quality of the deployed model can have a large financial impact.
...
An efficient deployment platform, aside from being robust and easy to tune, must be able to maintain the quality of the deployed model without incurring the high training cost of the periodical deployment approach.
 ...
 \todo[inline]{Solution}
We propose a deployment platform, that completely eliminates the need for retraining, thus greatly reducing the training cost, while achieving the same level of quality that the periodical deployment approach reaches.
