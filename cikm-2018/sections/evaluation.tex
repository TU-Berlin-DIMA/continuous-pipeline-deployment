...
Figure \ref{quality-vs-time} shows the ratio of increase in quality and end-to-end deployment time when compared with the baseline deployment approach (where no model training is performed).
The X-axis represents the baseline classifier (0 increase in the quality).
The values on the Y-axis represents the goodness of a deployment approach, where a value of 1 shows a perfect model that has an error rate of 0 and has the same end-to-end deployment time as the baseline approach.
...
However, due to the extremely long training time, the ratio of quality and end-to-end time is almost close to 0.
On the other hand, the continuous deployment approach guarantees high-quality models using a smaller training time (up to 1 order of magnitude) resulting in a higher quality/time ratio.
...
\caption{Trade-off between model quality and total training time. Each point represent the increase in quality-time over the baseline classifier (x-axis) and a perfect classifier (error rate of 0 with same training time as the baseline classifer)}
