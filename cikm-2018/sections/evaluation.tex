...
Instead of using constant learning rate, we use advanced learning rate adaptation techniques (Adam, Adadelta, and RMSPROP).
...
For the URL pipeline, Adam with learning rate $0.01$ and the regularization parameter $0.001$ has the best performance, followed by Rmsprop and Adadelta.
...
Continuous deployment approach can reduce the end-to-end deployment time by 1 order of magnitude.However, continuous deployment approach may reduce the quality.
To understand the trade-off between quality and the training time, we define a performance indicator ($PI$) as:\\
\begin{center}
$PI(t) = \dfrac{err_b(t) - err(t)}{cost(t) - cost_b(t)} * 100$
\end{center}
where $err(t)$ and $cost(t)$ are the cumulative error rate and the cumulative deployment cost (training time + prediction time) at a time $t$ and  $err_b(t)$ and $cost_b(t)$ are the error rate and the total deployment cost of the baseline deployment approach(only prediction time).
Figure \ref{quality-vs-time} shows the value of $PI$ over time for the continuous and periodical deployment approach.
...
However, due to the extremely long training time, the cumulative cost of the deployment is increasing exponentially with every periodical training.
...
On the other hand, the continuous deployment approach guarantees high-quality models with only a linear increase in the cumulative deployment cost ( the training time for every proactive training is constant) which results in a slow increase the value of $PI$ over time. 

\begin{figure}[!h]
\centering
\resizebox{\columnwidth}{!}{\input{../images/experiment-results/tikz/quality-vs-time}}
\caption{Trade-off between model quality and total training time. The $PI$ for baseline is always 0 (no increase in quality over the baseline). A $PI$ of close to 100\% indicates an Oracle deployment approach where the deployed model has an error rate of 0 and deployment cost of similar to the baseline.}
