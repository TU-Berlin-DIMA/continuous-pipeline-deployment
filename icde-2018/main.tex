...
A machine learning pipeline is the result of a complex data analytics process where an expert designs a set of operations to create complex predictive models from a training dataset.
To fully utilize such a pipeline, it has to be deployed into an environment where it can answer prediction queries in real-time.
Continuous training and maintenance of such pipelines is necessary to guarantee an acceptable prediction accuracy.
...
%The most common optimization technique to train machine learning models is Stochastic Gradient Descent (SGD) \cite{bottou2010large} as it is able to cater both large and real-time workloads.
...
To maximize the benefit for the ads provider, publisher, and the advertising network itself, advertising networks use machine learning pipelines to estimate the click rate of different ads.
...
\begin{itemize}
\item A \textbf{missing value imputer} that replaces missing values with appropriate values
\item A \textbf{label indexer} that finds the different unique values in a categorical feature 
\item A \textbf{one-hot encoder} that creates a new binary feature for every unique value in a categorical feature
\item A \textbf{data bucketizer} that transforms continuous variables into a series of binary variables
\item A \textbf{standard scaler} that scales the data columns to have unit standard deviation and zero mean
\item A \textbf{logistic regression model} trained using Stochastic Gradient Descent (SGD) optimization technique
\end{itemize}

Once this pipeline is created, it is deployed into a platform to be used in production.
...
One or a few ads with the highest click rate estimates are then shown to the user.
Depending on whether the user clicks on the ad or not, feedback is sent to the deployment platform.
Based on this feedback, the pipeline is continuously updated.
...
This example demonstrates the complex work flow of a deployment platforms.
Moreover, the deployment platform must be able to guarantee predictions with high accuracy and low latency, all the while accommodating  all the requests and new training data arriving at the system. 
...
In both operation mode, the overhead of offline training is large and updated pipelines are not immediately available in the deployment environment.
As a result, a high quality of predictions cannot be guaranteed at all time.
...
As a result, any information gathered during the online processing is not available for the offline training.

\textbf{Solution.} 
We found that during the prediction query answering and real-time learning, various statistics can be collected which will speed up the offline training of the pipeline.
Moreover, instead of a reactive offline training of the pipeline, we can update the pipeline proactively in smaller batches. 
...
\item Remove the overhead of reactive offline training by replacing it with proactive mini-batch training
\end{itemize}
