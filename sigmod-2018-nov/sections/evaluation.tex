...
Figure \ref{fig:sampling-mode-quality} shows how different sampling modes, namely random sampling, time based random sampling, and no sampling, affects quality of the model in Criteo pipeline.
...
We evaluated the quality of the model for one day and half a day time intervals.
...
In our experiment, using a time interval of half a day for the time based sampling, results in an error rate that is 0.14\% smaller than when using the simple random sampling. 
...
Our experiment shows that enabling both online statistics update and materialization reduces the total training time by a factor of $18$, from $8530$ to $470$.
...
As a result, the size of the materialized data increased by a nearly a factor of 2.
