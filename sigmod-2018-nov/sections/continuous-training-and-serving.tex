...
In this section, we describe our continuous training approach.
We describe the optimization technique for training the deployed model.
We also provide a detailed description of how we implement each of the proposed optimizations (proactive training and online statistics computation and data materialization).
...
\todo[inline]{But then there are other concerns: 1. Can we offer automatic or even manual way of fixing whatever the problem is with the model training? 2. The overhead it has on the system? This should be very small only depends on the size of the evaluation set.}
