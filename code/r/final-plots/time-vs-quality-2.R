...
urlDataProcessing <- function(){
  ...
  #onlineTime = cumrep(oTime/120,120)
  ...
  # Quality improvement
  df[c('Continuous','Periodical','Baseline')]  = (baseline$mc - df[c('Continuous','Periodical','Baseline')]) / baseline$mc
  ...
  #df$PerfectClassifier = df$PerfectClassifier/baselineTime
  df$Periodical = 100* df$Periodical/periodicalTime
  df$Baseline = 100*df$Baseline/baselineTime
