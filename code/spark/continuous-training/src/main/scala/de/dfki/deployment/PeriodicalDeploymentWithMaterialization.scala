...
class PeriodicalDeploymentWithMaterialization(val history: String,
  ...
  override def deploy(streamingContext: StreamingContext, pipeline: Pipeline) = {
    ...
    materializedHistory.map(dataParser.unparsePoint).repartition(spark.defaultParallelism).saveAsTextFile(s"$materializedLocation/0")
    ...
    for (day <- days) {
      ...
      materialized.map(dataParser.unparsePoint).repartition(spark.defaultParallelism).saveAsTextFile(s"$materializedLocation/$i")
      i += 1
