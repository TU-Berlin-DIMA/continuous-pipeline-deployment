...
object ScheduledExample {
  ...
  def main(args: Array[String]): Unit = {
    ...
    var observations = new BatchFileInputDStream[LongWritable, Text, TextInputFormat](ssc, streamingDataPath).map(_._2.toString).map(parsePoint)
    ...
    val task = new Runnable {
      def run() = {
        ...
        observations = new BatchFileInputDStream[LongWritable, Text, TextInputFormat](ssc, streamingDataPath).map(_._2.toString).map(parsePoint)
        observations.print()
        ssc.start()
